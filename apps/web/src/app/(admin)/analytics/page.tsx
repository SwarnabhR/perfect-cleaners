...
// Build time-bucket labels + bucket function for each range
...
function bucketConfig(r: RangeKey): BucketConfig {
  ...
  if (r === '30D') {
    ...
    const labels = Array.from({ length: 6 }, (_, i) => {
      const d = new Date(); d.setDate(d.getDate() - (25 - i * 5));
    ...
    return {
      ...
      bucket: (d: Date) => {
        const diff = Math.floor((now.getTime() - d.getTime()) / 86400000);
        const b = 5 - Math.floor(diff / 5);
        return b >= 0 && b < 6 ? b : -1;
      },
    };
  }
  if (r === '90D') {
    // 9 buckets of 10 days each
    const labels = Array.from({ length: 9 }, (_, i) => {
      const d = new Date(); d.setDate(d.getDate() - (80 - i * 10));
      return d.toLocaleString('en-IN', { month: 'short', day: 'numeric' });
    });
    return {
      labels,
  ...
  return {
    ...
    bucket: (d: Date) => {
      for (let i = 0; i < 12; i++) {
        const ref = new Date(); ref.setMonth(ref.getMonth() - (11 - i));
        if (d.getMonth() === ref.getMonth() && d.getFullYear() === ref.getFullYear()) return i;
      }
      return -1;
    },
  };
...
export default function AnalyticsPage() {
  ...
  // ── Range filter ─────────────────────────────────────────────────────────
  // All downstream derivations consume `filtered`, not `bookings` directly.
  const filtered = useMemo(() => {
    const cutoff = rangeStart(range);
    if (!cutoff) return bookings;
    return bookings.filter(b => {
      const ts = (b as any).scheduledAt ?? (b as any).createdAt;
      const d = toDate(ts);
  ...
  // Time-bucketed revenue for sparkline
  ...
  const bucketRevenue = useMemo(() => {
    ...
    doneBookings.forEach(b => {
      ...
      if (!d) return;
      const idx = bucket(d);
      if (idx >= 0) arr[idx] += b.priceBreakdown?.total ?? 0;
    });
    return arr;
  ...
  const topServices = Object.entries(svcRevenue)
    ...
    .map(([svc, { jobs, revenue }]) => ({
      ...
      jobs, revenue, avg: jobs > 0 ? Math.round(revenue / jobs) : 0,
    }));
            ...
            <svg aria-hidden="true" viewBox="0 0 800 220" style={{ width: '100%', height: '100%', overflow: 'visible' }} preserveAspectRatio="none">
              ...
              {pts && (
                <>
                  ...
                  {bucketRevenue.map((v, i) => {
                    const max = Math.max(...bucketRevenue, 1);
                    const x = Math.round((i / (bucketRevenue.length - 1)) * 800);
              ...
              {!pts && !loading && (
                <text x="400" y="110" textAnchor="middle" fill="var(--pc-fg-4)" fontSize="14" fontFamily="sans-serif">No completed bookings in this period</text>
              )}
            </svg>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 8 }}>
            {bucketLabels.map((m, i) => (
              ...
              {topServices.map((row, i) => {
                ...
                return (
                  <tr key={row.service} style={{ borderBottom: i < topServices.length - 1 ? '1px solid var(--pc-line)' : 'none' }}>
                    ...
                    <td style={{ padding: '12px 20px', fontFamily: 'var(--pc-sans)', fontSize: 14, color: 'var(--pc-fg)', fontWeight: 600 }}>₹{row.revenue.toLocaleString('en-IN')}</td>
                    <td style={{ padding: '12px 20px', fontFamily: 'var(--pc-sans)', fontSize: 14, color: 'var(--pc-fg-2)' }}>₹{row.avg.toLocaleString('en-IN')}</td>
