...
export default function BookingsPage() {
  ...
  const [assigning,  setAssigning]  = useState<string | null>(null); // bookingId
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, snap => {
      ...
      setLoading(false);
    }, err => { console.warn('[Bookings] Firestore:', err.message); setLoading(false); });
    return () => unsub();
  }, []);
  ...
  async function assignWorker(bookingId: string, worker: LiveWorker) {
    try {
      ...
      console.error('[Bookings] assign failed:', err.message);
    }
  }

  const filtered = filter === 'All'
    ? bookings
    : bookings.filter(b => STATUS_LABELS[b.status] === filter);
              ...
              {filtered.map(b => (
                <tr key={b.id} style={{ borderBottom: '1px solid var(--pc-line)' }}>
                  ...
                  <td style={{ padding: '13px 18px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ width: 28, height: 28, borderRadius: 999, background: 'var(--pc-card-hi)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <span style={{ fontFamily: 'var(--pc-sans)', fontSize: 11, fontWeight: 600, color: 'var(--pc-fg-2)' }}>
                          {(b.customerName ?? b.customerId)?.[0]?.toUpperCase() ?? '?'}
                        </span>
                      </div>
                      <span style={{ fontFamily: 'var(--pc-sans)', fontSize: 14, color: 'var(--pc-fg)' }}>
                        {b.customerName ?? b.customerId.slice(0, 8)}
                      </span>
                    </div>
                  </td>
                  <td style={{ padding: '13px 18px', fontFamily: 'var(--pc-sans)', fontSize: 13, color: 'var(--pc-fg-2)' }}>
                    {serviceLabel(b.serviceIds)}
                  </td>
                  <td style={{ padding: '13px 18px', fontFamily: 'var(--pc-sans)', fontSize: 13, color: 'var(--pc-fg-2)' }}>
                  ...
                  <td style={{ padding: '13px 18px' }}>
                    {b.workerName ? (
                      ...
                      <button
                        ...
                        style={{
                          padding: '5px 12px', borderRadius: 999, border: '1px solid var(--pc-warning)',
                          background: 'transparent', color: 'var(--pc-warning)',
                    ...
                    {assigning === b.id && (
                      <div style={{
                        position: 'absolute', zIndex: 50, marginTop: 8,
                        background: 'var(--pc-card)', border: '1px solid var(--pc-line)',
                        borderRadius: 10, padding: 8, minWidth: 200,
                        boxShadow: '0 8px 24px rgba(0,0,0,0.3)',
                      }}>
                        <p style={{ fontFamily: 'var(--pc-mono)', fontSize: 10, color: 'var(--pc-fg-3)', padding: '4px 8px', margin: 0 }}>
                          SELECT WORKER
                        </p>
                        {workers.length === 0 ? (
                          ...
                          >
                            <div style={{ width: 24, height: 24, borderRadius: 999, background: 'var(--pc-sage)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <span style={{ fontFamily: 'var(--pc-sans)', fontSize: 10, fontWeight: 600, color: '#fff' }}>
                                {w.name[0]}
                              </span>
                            </div>
                            <div style={{ flex: 1 }}>
                  ...
                  <td style={{ padding: '13px 18px', fontFamily: 'var(--pc-sans)', fontSize: 14, color: 'var(--pc-fg)', fontWeight: 600 }}>
                    ₹{b.priceBreakdown?.total?.toLocaleString('en-IN') ?? '—'}
                  </td>
                  <td style={{ padding: '13px 18px' }}>
                    <StatusPill status={STATUS_LABELS[b.status] ?? b.status} />
                  </td>
                  <td style={{ padding: '13px 18px' }} />
                </tr>
              ))}
