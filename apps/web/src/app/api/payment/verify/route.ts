...
// Razorpay is disabled for now — no live account/keys, and payment collection
// currently happens by phone (admin calls the customer), not online
// self-checkout. This endpoint can no longer be reached anyway once
// create-order (its only caller's first step) is disabled; kept in place,
// real logic commented out, so it's a straightforward flip back on once real
// keys exist.
