      ...
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={s.fieldArea}>
          ...
          <View style={s.colorRow}>
            {COLORS.map(c => (
              <TouchableOpacity
                key={c.label}
                style={[s.colorSwatch, { backgroundColor: c.hex }, carColor === c.label && s.colorSwatchActive]}
                onPress={() => setCarColor(c.label)}
                activeOpacity={0.8}
              >
                {carColor === c.label && (
                  <Check size={12} color={c.label === 'White' || c.label === 'Silver' ? '#000' : '#fff'} strokeWidth={2.5} />
                )}
              </TouchableOpacity>
            ))}
          </View>
        ...
        <TouchableOpacity
          ...
          onPress={() => ready && router.push({
            ...
            params: { name, make, model, plate, carColor },
          })}
          activeOpacity={0.8}
          disabled={!ready}
        >
...
const s = StyleSheet.create({
  ...
  chipActive: { backgroundColor: colors.warm, borderColor: 'transparent' },
  chipText: { fontFamily: typography.sans, fontSize: 12, color: colors.fg2 },
  chipTextActive: { color: colors.ink, fontFamily: typography.sansMedium },
  ...
  colorSwatchActive: { borderColor: colors.lineStrong },
  colorLabel: { fontFamily: typography.mono, fontSize: 10, color: colors.fg2, letterSpacing: 0.6 },
