...
export default function BookingScreen() {
  ...
  return (
    <View style={[s.root, { backgroundColor: c.ink, paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      <ScrollView contentContainerStyle={s.scrollContent} showsVerticalScrollIndicator={false}>

        {/* Header */}
        <View style={s.header}>
          <TouchableOpacity
            style={[s.backBtn, { backgroundColor: c.card, borderColor: c.line }]}
            onPress={() => router.back()}
          >
            <ChevronLeft size={16} color={c.fg} strokeWidth={1.5} />
          </TouchableOpacity>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>[BOOKING] / NEW</Text>
        </View>

        <View style={s.titleSection}>
          <Text style={[s.title, { color: c.fg }]}>Choose your wash.</Text>
        </View>
        ...
        <View style={s.section}>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>[STEP 01] · SELECT PACKAGE</Text>
            ...
            {PACKAGES.map(p => {
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    s.pkgCard,
                    { backgroundColor: selected ? c.cardHi : c.card,
                      borderColor:      selected ? c.lineStrong : c.line },
                  ]}
        ...
        {/* Step 02 — Time slot */}
        <View style={s.section}>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>[STEP 02] · PICK A TIME · TUE, 28 MAY</Text>
        ...
        {/* Step 03 — Vehicle */}
        <View style={s.section}>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>[STEP 03] · YOUR VEHICLE</Text>
          <DetailCard
        ...
        {/* Step 04 — Address */}
        <View style={s.section}>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>[STEP 04] · SERVICE ADDRESS</Text>
          <DetailCard
      ...
      <View style={[s.stickyBar, { backgroundColor: c.ink, borderTopColor: c.line }]}>
        <View style={s.stickyInfo}>
          <Text style={[s.eyebrow, { color: c.fg3 }]}>TOTAL</Text>
          <Text style={[s.stickyTotal, { color: c.fg }]}>{activePkg.price}</Text>
        </View>
        <HapticButton
          haptic="medium"
          style={[s.stickyBtn, { backgroundColor: c.warm }]}
          onPress={() => router.push('/(customer)/payment')}
          activeOpacity={0.85}
        >
          <Text style={[s.stickyBtnText, { color: c.ink }]}>Confirm Booking →</Text>
        </HapticButton>
      </View>
    </View>
  );
...
// Zero color values — all theme colors applied as inline style overrides in JSX.
const s = StyleSheet.create({
  root:          { flex: 1 },
  scrollContent: { paddingBottom: 120 },

  header:  { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: spacing[5], paddingVertical: spacing[3] },
  backBtn: { width: 36, height: 36, borderRadius: 999, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  ...
  title:        { fontFamily: typography.serif, fontSize: 32, letterSpacing: -0.3, lineHeight: 34 },
  ...
  pkgCard:     { borderWidth: 1, borderRadius: radii.md, padding: spacing[4], gap: spacing[2] },
  pkgHead:     { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'baseline' },
  pkgName:     { fontFamily: typography.sansMedium, fontSize: typography.base },
  pkgPrice:    { fontFamily: typography.sansSemiBold, fontSize: typography.lg },
  pkgFeatures: { gap: 6 },
  pkgFeature:  { flexDirection: 'row', gap: 8, alignItems: 'center' },
  pkgFeatureText: { fontFamily: typography.sans, fontSize: 12 },
  pkgSelected: { fontFamily: typography.mono, fontSize: 9, letterSpacing: 1, alignSelf: 'flex-end' },

  slotRow:  { flexDirection: 'row', gap: 8, marginTop: spacing[2], flexWrap: 'wrap' },
  slotChip: { paddingVertical: 10, paddingHorizontal: 14, borderRadius: 10, borderWidth: 1 },
  slotText: { fontFamily: typography.sans, fontSize: 13 },

  detailCard:    { flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: spacing[2], borderWidth: 1, borderRadius: radii.md, padding: spacing[4] },
  detailIcon:    { width: 40, height: 40, borderRadius: 10, borderWidth: 1, alignItems: 'center', justifyContent: 'center' },
  detailInfo:    { flex: 1 },
  detailPrimary: { fontFamily: typography.sansMedium, fontSize: typography.base },
  detailSub:     { fontFamily: typography.sans, fontSize: 12, marginTop: 2 },

  stickyBar:     { flexDirection: 'row', alignItems: 'center', gap: 12, paddingHorizontal: spacing[5], paddingTop: spacing[3], paddingBottom: spacing[3], borderTopWidth: 1 },
  stickyInfo:    { flex: 1 },
  stickyTotal:   { fontFamily: typography.serif, fontSize: typography['2xl'] },
  stickyBtn:     { borderRadius: radii.pill, paddingVertical: 14, paddingHorizontal: 22 },
  stickyBtnText: { fontFamily: typography.sansSemiBold, fontSize: 13, letterSpacing: 0.6 },
});
